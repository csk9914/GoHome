...
| 질문 | 참조 문서 |
|---|---|
| 어떤 클래스/인터페이스를 만들어야 해? 다른 시스템과 어떻게 연결돼? (**메인 개발 루프에서 가장 흔한 질문**) | 클래스 자체는 `Source/GoHome/<담당 폴더>/`가 출처. 경계 규칙·이유는 [시스템별 결정과 경계](ARCHITECTURE.md#시스템별-결정과-경계) + [시스템 간 인터페이스 계약](ARCHITECTURE.md#시스템-간-인터페이스-계약) — 여기서 안 풀리면 그때만 아래 "기획 의도/수치 근거" 행 참고 |
