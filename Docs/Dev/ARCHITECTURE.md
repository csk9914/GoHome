...
폴더별로 코드가 강제하지 않는 결합도 경계와 결정 이유만 다룬다. 클래스/필드/구현 상태는 `Source/GoHome/<폴더>/`가 유일한 출처다.
...
decisions:
  - name: HP 0 처리 (설계 의도 — GameState 구독부/Fail 모두 현재 빈 스텁)
...
| `GenerateNoise` | AI | Item(구현됨, `ItemActorBase.cpp`)이 발생원, Player(이동 사운드)는 계획상 발생원이나 미구현. 호출 즉시 서버 동기 처리 | 02문서 2·4·5절 |
| `IMonsterNoiseListener` (신설 제안) | AI | `AMonsterBase`가 구현, `GenerateNoise`가 반경 내 각 몬스터에 동기 호출(반응 로직은 구현체 내부, 경계 밖) | `GenerateNoise`는 호출까지만 책임지고 전이 로직은 몬스터 구현체가 정한다(경계 밖) |
...
| `OnDeath` (신설 제안) | Player | Core(설계 의도 — GameState가 탐사마다 재구독해 생존자 수 추적, 0명이면 Fail. 구독부/Fail 모두 빈 스텁) | HP 0 처리 결정(UHealthComponent 소유) |
