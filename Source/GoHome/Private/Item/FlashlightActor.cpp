...
void AFlashlightActor::Tick(float DeltaTime)
{
	if (HoldingPawn)
	{
		// 손에 들린 동안은 부력 로직(Super) 대신, 빛 회전을 플레이어 시야 방향으로 계속 맞춰줌.
...
void AFlashlightActor::OnInteract(APawn* InstigatorPawn)
{
	...
	// 손전등은 슬롯은 차지하되, 활성 슬롯 여부와 무관하게 항상 가슴에 부착.
	// 양손이 자유로우니 오른손 아이템/협동 운반과도 동시 사용 가능함.
	UpdateAttachment();
}

void AFlashlightActor::UpdateAttachment(APawn* OldHoldingPawn)
{
	if (HoldingPawn)
	{
		MeshComponent->SetVisibility(true, false);
		MeshComponent->SetCollisionEnabled(ECollisionEnabled::NoCollision);
		MeshComponent->SetCastShadow(false); // 손전등 그림자 생기는 것 방지.
		...
		// 빛 회전 동기화용으로 Tick은 계속 켜두어야 함(서버-클라 전부 로컬로 계산 -> HasAuthority 밖에서 호출).
		SetActorTickEnabled(true);

		if (AGoHomeCharacter* Character = Cast<AGoHomeCharacter>(HoldingPawn))
		{
			Character->AttachFlashlightToChest(MeshComponent);
		}
	}
	...
	else
	{
		...
		MeshComponent->SetCastShadow(true); // 월드에 놓인 손전등은 다시 정상적으로 그림자 생김.

		if (HasAuthority())
		{
			MeshComponent->SetSimulatePhysics(true);

			bIsOn = false; // 드롭하면 항상 꺼짐.
			UpdateLightVisual();
		...
		{
			PrevCharacter->DetachFlashlightFromChest();
		}
	}
}
